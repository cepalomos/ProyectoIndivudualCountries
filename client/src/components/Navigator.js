...
export default function Navigator() {
  ...
  const [actividad, setActividad] = useState([
    { id: 1, name: "no hay opciones" },
  ]);
  useEffect(() => {
    let continente = new Set(countries.map(({ continente }) => continente));
    continente = Array.from(continente).map((dato) => ({
      id: dato,
      name: dato,
    }));
    setContinente(continente);
    let actividad = new Set(
      countries
        .map(({ activities }) => (activities.lenght !== 0 ? activities : false))
        .flat()
        .filter((obj) => obj)
        .map(({ nombre }) => nombre)
    );
    ...
    <>
      <nav className="navigator_contenedor">
        ...
        <div className="navigator_selectores">
          <Order funcion={order} />
          <Filter
            opcionContinente={continente}
            funcionContinente={filterContinente}
            funcionActividad={filterActividad}
            opcionActividad={actividad}
          />
        </div>
        <Input />
      </nav>
